...
<head>
    ...
    <title>Bowling_Statistics</title>
     <link rel='stylesheet' href="select_styles.css">
</head>
<body>
<h2 style="padding-top:2rem;padding-bottom:2rem;text-align: center"><strong>Bowling_Statistics</strong></h2>
  ...
  <thead>
   ...
   // $sql="SELECT pname,team,date_of_birth,age,coach_name from `testing_val`,`batting_coach` where pid=coach_id and age>1850";
    $sql2="SELECT `bowler_details`.player_name,player_team,matches,economy,wickets from `bowler_details`,`bowling_statistics` where `bowling_statistics`.player_name=`bowler_details`.player_name";
   ...
   // {
        ...
        while($row2=mysqli_fetch_assoc($result2))
        {
             ...
             echo '<tr>';
             echo '<td>'.$row2['player_name'].'</td>';
             //echo '<td>'.$row['pid'].'</td>';
             echo '<td>'.$row2['player_team'].'</td>';
             echo '<td>'.$row2['matches'].'</td>';
             echo '<td>'.$row2['economy'].'</td>';
             echo '<td>'.$row2['wickets'].'<td>';
             echo '<tr>';
             //echo '<br>';
        }
        //echo '<br>';
